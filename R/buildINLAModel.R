...
buildINLAmodelV2 <- nimbleFunction(
  name = 'buildINLAmodelV2',
  setup = function(model, fam, x, y, control) {
    inlaModel <- extractControlElement(control, 'fit.inla',  NULL)
    fixedVals <- extractControlElement(control, 'fixedVals',  double())
    ...
    inlaStepFunctions <- nimbleFunctionList(inlaStepVirtualV2)

    if(multiple == TRUE){
      inlaStepFunctions[[1]] <- inlaFStepMultipleV2(model,
                                                  mvEWSamples,
                                                  fixedVals,
                                                  # inlaModel,
                                                  x,
      ...
      inlaStepFunctions[[1]] <- inlaFStepV2(model,
                                          mvEWSamples,
                                          fixedVals,
                                          # inlaModel,
                                          x,
  ...
  run = function(beta=double(1), # beta is a vector
    ...
    out <- inlaStepFunctions[[1]]$run(beta, extraVars)#, interInModel)
    #rr <- inlaStepFunctions[[1]]$mvEWSamples
    logL <- out
    if(logL == -Inf) {lastLogLik <<- logL; return(logL)}
    if(is.nan(logL)) {lastLogLik <<- -Inf; return(-Inf)}
