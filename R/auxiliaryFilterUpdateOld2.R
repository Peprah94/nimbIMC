setSavedWts1 <-  function(mv, currIndx, indx, mvWSamplesXSaved){
...
auxFStepUpdate1 <- nimbleFunction(
  name = 'auxFStepUpdate1',
  ...
  run = function(m = integer()) {
    ...
    #save the log likelihoods for updating
...
#     for(iNode in seq_along(nodes))
#       auxStepFunctions[[iNode]] <- auxFStepUpdate(model, mvEWSamples, mvWSamples,
#                                             nodes, iNode, names, saveAll,
#                                             smoothing, lookahead,
#                                             resamplingMethod, silent)
#
